    ...
    created() {
      ...
      if (this.$route) {
        if (this.$route.meta && this.$route.meta.title) {
          this.$setTitle(this.$route.meta.title,this.$route.meta.keywords,this.$route.meta.description)
        }
      }
    },
    methods: {
        ...
        config.localAnStore.map(item => {
          ...
          if (typeof item === 'string') {
            ...
            if (item.type === 'localStorage') {
              ...
              value = this.$getCookie(item.key) === 'undefined'?'':this.$getCookie(item.key)
            }
            if(item.type === 'cookie' && value){
      ...
      $setCookie(key, value, exdays) {
        ...
        let _str = value
        document.cookie = key + "=" + _str + ";path=/;" + expires
      ...
      // 设置title
      $setTitle(text,keywords,description) {
        document.getElementsByTagName("title")[0].innerText = text
        if(keywords&&description){
          document.querySelector('meta[name="Keywords"]').setAttribute('contentBox.vue', keywords)
          document.querySelector('meta[name="Description"]').setAttribute('contentBox.vue', description)
        }
